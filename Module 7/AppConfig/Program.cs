...
internal class Program
{
    ...
    const string conStr = "Endpoint=https://ps-config.azconfig.io;Id=dr7G;Secret=3c9ssv2iKGvDGra0JpQiJ2eRaLnNT2WFjm1gLLLIQmxPYFNddGIuJQQJ99CDAC5RqLJJs58ZAAACAZAC4PNp";
    ...
    private static Task ReadConfigurationLocalAsync()
    {
        // From user-secrets
        // dotnet init
        // dotnet user-secrets set APPC "Endpoint="
    ...
    {
        var builder = new ConfigurationBuilder();
        //builder.AddUserSecrets<AppConfig.Program>(true)
        ...
        builder.AddAzureAppConfiguration(opts =>
        {
            ...
            opts.Connect(conStr)
                .Select("*", "Test").TrimKeyPrefix("KlantA:");
              //  .TrimKeyPrefix("");//.Select("*", "Production").TrimKeyPrefix("AppConvig:");//.Select("*", "Production").TrimKeyPrefix("AppConfig:MySetings:");
       ...
       Console.WriteLine(conf["MySetings:hello"]);
        Console.WriteLine(conf["KeyVault:BackgroundColor"]);
